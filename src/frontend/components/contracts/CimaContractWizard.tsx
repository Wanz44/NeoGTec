...
export const CimaContractWizard: React.FC<CimaContractWizardProps> = ({ onBackToOffers, logAction }) => {
  ...
  const [plansTarifaires, setPlansTarifaires] = useState<string[]>(['Gold standard', 'Silver pro', 'Bronze basic']);
  const [selectedOffre, setSelectedOffre] = useState('Gold standard');

  React.useEffect(() => {
    async function fetchPlans() {
      try {
        const { data, error } = await supabase.from('plans_tarifaires').select('nom');
        if (data && !error) {
          setPlansTarifaires(data.map((p: any) => p.nom));
          if (data.length > 0) setSelectedOffre(data[0].nom);
        }
      } catch (e) {
        console.error("Error fetching plans_tarifaires:", e);
      }
    }
    fetchPlans();
  }, []);
