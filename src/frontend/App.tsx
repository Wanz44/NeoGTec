...
const FAQSection = () => (
  <div className="flex flex-col items-center justify-center py-20 text-center bg-orange-50/20 rounded-fluent border border-dashed border-orange-200">
    <h3 className="text-xl font-bold text-orange-950 mb-2">Aide & FAQ</h3>
    <p className="text-orange-950/50 max-w-sm">Le centre d'aide est actuellement vide.</p>
  </div>
);
...
const SearchInput = () => (
  <div className="relative group flex-1 max-w-xl">
    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-6 h-6 text-orange-600 group-focus-within:text-orange-700 transition-colors stroke-[2.5px]" />
    <input 
      type="text" 
      placeholder="Rechercher une police, un prestataire ou un dossier..." 
      className="w-full pl-12 pr-4 py-3 bg-white/40 backdrop-blur-sm border border-orange-200/30 rounded-sm focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 outline-none transition-all placeholder:text-orange-950/20 text-orange-950 font-medium"
    />
    <div className="absolute right-4 top-1/2 -translate-y-1/2 flex gap-1 pointer-events-none">
      <span className="px-1.5 py-0.5 rounded border border-orange-200 text-[10px] font-bold text-orange-500 bg-white/80">⌘</span>
      <span className="px-1.5 py-0.5 rounded border border-orange-200 text-[10px] font-bold text-orange-500 bg-white/80">K</span>
    </div>
  </div>
);
...
export default function App() {
      ...
      default: return ( // Fallback pour les modules non encore implémentés (Placeholders)
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-20 h-20 bg-orange-50 rounded-full flex items-center justify-center mb-6">
            <Info className="w-10 h-10 text-orange-500/40" />
          </div>
          <h3 className="text-xl font-bold text-orange-950 mb-2">Module en Développement</h3>
          <p className="text-orange-950/50 max-w-sm">
  ...
  return (
    <div className="flex h-screen bg-brand-beige text-slate-900 font-sans selection:bg-orange-500 selection:text-white overflow-hidden relative p-4 gap-4">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative z-10 rounded-2xl bg-white border border-white/40 shadow-xl">
        ...
        <header className="h-16 px-8 flex items-center justify-between bg-white/90 backdrop-blur-md border-b border-orange-100/50 sticky top-0 z-50">
          <SearchInput />
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative group">
              <button className="p-2 bg-white/50 rounded-sm border border-white/20 text-orange-900/60 hover:bg-white/80 transition-all relative outline-none active:scale-95">
              ...
              <div className="absolute top-full right-0 mt-2 w-72 material-mica rounded-md p-4 opacity-0 scale-95 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:scale-100 group-hover:translate-y-0 transition-all z-50 border border-white/60">
                ...
                <div className="space-y-2">
                  <div className="flex gap-3 p-2 rounded-[8px] hover:bg-orange-50/50 transition-colors cursor-default">
                    ...
                    <p className="text-[11px] text-orange-950/80 leading-relaxed font-medium">Contrat GOUV-2024 en attente de signature électronique (Tenant: AS-912).</p>
                  </div>
                  <div className="flex gap-3 p-2 rounded-[8px] hover:bg-orange-50/50 transition-colors cursor-default">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                    <p className="text-[11px] text-orange-950/80 leading-relaxed font-medium">Audit RGPD hebdomadaire : 100% conformité détectée sur le module Operational.</p>
                  </div>
                </div>
              </div>
            </div>
